...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-8">
        <h1 class="text-3xl font-bold text-saffron-800">Edit Festival</h1>
    ...
    <form action="{{ route('superadmin.festivals.update', $festival) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-md p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $festival->name) }}" required
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-saffron-500 focus:ring focus:ring-saffron-200">{{ old('description', $festival->description) }}</textarea>
                </div>

                <div>
                    <label for="deity" class="block text-sm font-medium text-gray-700">Deity</label>
                    <input type="text" name="deity" id="deity" value="{{ old('deity', $festival->deity) }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-saffron-500 focus:ring focus:ring-saffron-200">
                </div>

                <div>
                    <label for="date" class="block text-sm font-medium text-gray-700">Date</label>
                    <input type="date" name="date" id="date" value="{{ old('date', $festival->date->format('Y-m-d')) }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-saffron-500 focus:ring focus:ring-saffron-200">
                </div>

                <div>
                    <label for="duration" class="block text-sm font-medium text-gray-700">Duration</label>
                    <input type="text" name="duration" id="duration" value="{{ old('duration', $festival->duration) }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-saffron-500 focus:ring focus:ring-saffron-200">
                </div>
            </div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea name="significance" id="significance" rows="3" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-saffron-500 focus:ring focus:ring-saffron-200">{{ old('significance', $festival->significance) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="rituals" id="rituals" rows="3" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-saffron-500 focus:ring focus:ring-saffron-200">{{ old('rituals', $festival->rituals) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="customs" id="customs" rows="3" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-saffron-500 focus:ring focus:ring-saffron-200">{{ old('customs', $festival->customs) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 space-y-4">
            <div>
                ...
                <textarea name="food" id="food" rows="2" required
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-saffron-500 focus:ring focus:ring-saffron-200">{{ old('food', $festival->food) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="clothing" id="clothing" rows="2" required
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-saffron-500 focus:ring focus:ring-saffron-200">{{ old('clothing', $festival->clothing) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="decorations" id="decorations" rows="2" required
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-saffron-500 focus:ring focus:ring-saffron-200">{{ old('decorations', $festival->decorations) }}</textarea>
            </div>
        </div>
        ...
        <div class="mt-6">
            ...
            @if($festival->featured_image)
                <div class="mt-2">
                    <img src="{{ Storage::url($festival->featured_image) }}" alt="{{ $festival->name }}" class="h-32 w-32 object-cover rounded-lg">
                </div>
            @else
        ...
        <div class="mt-6">
            <label for="gallery_images" class="block text-sm font-medium text-gray-700">New Gallery Images</label>
        ...
        <div class="mt-6 space-y-4">
            <div>
                <label class="flex items-center">
                    <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $festival->is_featured) ? 'checked' : '' }}
            ...
            <div>
                <label class="flex items-center">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $festival->is_active) ? 'checked' : '' }}
                        class="rounded border-gray-300 text-saffron-600 shadow-sm focus:border-saffron-300 focus:ring focus:ring-saffron-200 focus:ring-opacity-50">
